...
function profile_grid(model::AbstractGNFW{T}; Nr = 64, NlogM = 32, Nz = 32, zmin = 1e-3, zmax = 5.,
    logMmin = 11., logMmax = 16., minR = -10, maxR = 10., Nϵ₁ = 24, Nϵ₂ = 24, Nrot = 12)where T
    
    Rs = LinRange(minR, maxR, Nr)
    logMs = LinRange(logMmin, logMmax, NlogM)
    redshifts = LinRange(zmin, zmax, Nz)
...
function profile_grid(model::AbstractGNFW{T}, Rs, logMs, redshifts, ϵ₁, ϵ₂, ϕs, θs, ψs) where T
    ...
    dims = (Nr, Nr, Nz, NlogM, Nϵ₁, Nϵ₂, Nrot, Nrot, Nrot)
    println("N elements = ",prod(BigInt.(dims)))
    A = zeros(Float32, (Nr, Nr, Nz, NlogM, Nϵ₁, Nϵ₂, Nrot, Nrot, Nrot))
...
function build_interpolator(model::AbstractProfile; cache_file::String="", 
                            Nr=64, pad=256, overwrite=true, verbose=true)
